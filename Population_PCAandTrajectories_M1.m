...
load M1_A1_Data_100ms
load M1_PFC_Data_100ms
...
AC_Data = nan(sum(n_neurons_M1(1,:)),size(M1_PFC_Data_100ms{1, 1}  ,2)*12) ;  PFC_Data = nan(sum(n_neurons_M1(2,:)),size(M1_PFC_Data_100ms{1, 1}  ,2)*12) ;




for s=1: size (M1_A1_Data_100ms,2)

    clearvars -except s Acc_PFC Acc_AC M1_A1_Data_100ms NtrialsSessions PFC_Data AC_Data  Eig_Values_AC_Sess_Match  Eig_Values_AC_Sess Eig_Values_PFC_Sess Eig_Values_PFC_Sess_NonMatch ...
        nk k Times Smoothvalue SelectedDelays Times_fig M1_A1_Data_100ms  M1_PFC_Data_100ms n_neurons_M1  M1_BehavioralCodes M1_BehavioralData T_M  T_NonM  CRTR CLTL CRTR CRTL Monkey_acc_sessions

    size (M1_A1_Data_100ms,2) - s

    An_AC_Sess    = M1_A1_Data_100ms{s};
    An_PFC_Sess   = M1_PFC_Data_100ms{s};
    ...
    M_AC=[]; M_PFC=[];
    if s ==1 && Delay_s~=0
        AC_Data(1:n_neurons_M1(1,1),:)=M_AC_all_Del;
        PFC_Data(1:n_neurons_M1(2,1),:)=M_PFC_all_Del;
    elseif s >1 && Delay_s~=0
        AC_Data(sum(n_neurons_M1(1,1:s-1))+1:sum(n_neurons_M1(1,1:s)),:)=M_AC_all_Del;
        PFC_Data(sum(n_neurons_M1(2,1:s-1))+1:sum(n_neurons_M1(2,1:s)),:)=M_PFC_all_Del;
    end
    if s ==1 && Delay_s==0
        AC_Data(1:n_neurons_M1(1,1),:)=M_AC_all_Del;
        PFC_Data(1:n_neurons_M1(2,1),:)=M_PFC_all_Del;
...
score_PFC=smoothdata(score_pfc(:,1:k ),1,"gaussian",40);
