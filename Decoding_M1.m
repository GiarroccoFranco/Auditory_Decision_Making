...
Acc_PFC=nan(size (M1_A1_Data_100ms,2),size (M1_A1_Data_100ms{1},2),4);
Acc_AC=nan(size (M1_A1_Data_100ms,2),size (M1_A1_Data_100ms{1},2)),4;

% for s= 11
parfor s=1: size (M1_A1_Data_100ms,2)
    % for s= f
   ...
   Delay_Corr_All = union(union(Delay_Corr_1,Delay_Corr_2),Delay_Corr_3);
    ...
    for Del = 4
        ...
        elseif Delay_s  ==4
            ...
            PermData_AC=permute(An_AC_Sess,[3 1 2]);
            Accuracy_AC=[];
            Accuracy_AC=PerformDecodingTrials_AuditoryTask(PermData_AC,Delay_All,CueLocation(Delay_All));
            Acc_AC(s,:,Del)=Accuracy_AC;

            PermData_PFC=permute(An_PFC_Sess,[3 1 2]);
            Accuracy_PFC=[];
            Accuracy_PFC=PerformDecodingTrials_AuditoryTask(PermData_PFC,Delay_All,CueLocation(Delay_All));
            Acc_PFC(s,:,Del)=Accuracy_PFC;
